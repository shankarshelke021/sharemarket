...
class Angelbroking extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $start_date_time = ($request->date) ? date('Y-m-d H:s', strtotime('-1 day', strtotime($request->date . ' 9:00:00'))) : date('Y-m-d H:s', strtotime('-1 day', strtotime('9:15:00')));
        $end_date_time = ($request->date) ? date('Y-m-d H:s', strtotime($request->date . ' 15:30:00')) : date('Y-m-d H:s', strtotime('15:30:00'));
        ...
        if($GetCandleData['response_data']['data']){
        ...
        return view('forms.index', ['html_data' => $res]);
